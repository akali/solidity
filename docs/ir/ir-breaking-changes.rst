...
- Function modifiers are implemented in a slightly different way regarding function parameters and return variables.
  ...
  This means that multiple executions of a function's body will get the same values for the parameters,
  ...
  - Old code generator: ``1`` as the return variable is initialized to ``0`` only once before the first ``_;``
    ...
    its first value.
  - New code generator: ``0`` as all parameters, including return parameters, will be initialized again before
    each ``_;`` evaluation.
